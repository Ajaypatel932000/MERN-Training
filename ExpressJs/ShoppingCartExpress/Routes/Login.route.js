...
loginRoute.route('/').get(function(req,res){
     ...
     loginModel.find(function(err,employee){
         if(err) 
...
loginRoute.route('/addLogin').post(function(req,res){
    
     let loginData=new loginModel(req.body);
     console.log(req.body);
     loginData.save()
     .then(game=>{res.status(200).json({'LoginDetails':'LoginDetails Added Successfully'})})
